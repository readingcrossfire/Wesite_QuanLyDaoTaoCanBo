    ...
    <div class="row cRow">
        ...
        <div class="col-md-8 cColContent">
            <div class="theCourse">
                ...
                <form action="/" id="form-them-hocven" class="form-them-hocvien">
                    <div class="boxBorder">
                        <div class="form-group">
                            <label for="">Tên Lớp Học Viên</label>
                            <input type="text" class="form-control" name="ten-lop-hoc-vien" id="" aria-describedby="helpId" placeholder="Điền tên học viên">

                            <label for="">Số Lượng</label>
                            <input type="text" class="form-control" name="so-luong" id="" aria-describedby="helpId" placeholder="Điền chúc vụ">

                            <label for="">Tên Giảng Viên</label>
                            <input type="text" class="form-control" name="ten-giang-vien" id="" aria-describedby="helpId" placeholder="Điền phòng ban">
...
<script
    ...
    $(document).ready(function()
    {
        ...
        submit.click(function()
        {
            ...
            var tenlophocvien = $("input[name='ten-lop-hoc-vien']").val(); //lấy giá trị trong input user
            var soluong = $("input[name='so-luong']").val();
            var tengiangvien = $("input[name='ten-giang-vien']").val();
            //Kiểm tra xem trường đã được nhập hay chưa
            if(tenlophocvien == ''){
                alert('Vui lòng nhập Tên Lớp học viên');
                return false;
            }
            if(soluong == ''){
            ...
            var data = $('form-them-hocven').serialize();
            $("form").on('submit', function (e) {
                ...
                $.ajax({
                    ...
                    success : function(data)  // Hàm thực thi khi nhận dữ liệu được từ server
                    {
                        if(data == 'false')
                        {
                            alert('thêm học viên không thành công');
                        }else{
