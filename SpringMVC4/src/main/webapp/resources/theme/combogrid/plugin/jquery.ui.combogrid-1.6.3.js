    ...
    $.widget("cg.combogrid", {
        ...
        _create: function() {
            ...
            this.menucombo = $("<div></div>")
                    ...
                    .zIndex(this.element.zIndex() + 1000000000000)
...
//		ul2.appendTo(ul);
        ...
        _renderHeader: function(ul, colModel) {
            ...
            $.each(colModel, function(index, col) {
                ...
                var mAlign = col.headerAlign ? col.headerAlign : col.align;
                // Check if column is ordered or not to provide asc/desc icon
                if (col.columnName == self.cssCol) {
                    div.append('<div class="cg-colHeader" style="width:' + col.width + self.options.munit + ';' + hide + ' text-align:' + mAlign + '"><label class="cg-colHeader-label" id="' + col.columnName + '">'
        ...
        _renderItem: function(ul, item, colModel) {
            ...
            $.each(colModel, function(index, col) {
                ...
                var clazz = "cg-DivItem";
                if (col.class){
                    clazz += " " + col.class;
                }

                var mWidth = col.customItemWidth ? col.customItemWidth : col.width;
                var style = "width:" + mWidth + self.options.munit + ";"
                if (col.style){
                    style += col.style;
                }
                $("<div style='" + style + hide + " text-align:" + col.align + "; font-weight:" + col.weight + "; color:" + col.color + "' class='" + clazz + "'>" + colItem + "</div>").appendTo(div);
            });
            div.append("</div>");
